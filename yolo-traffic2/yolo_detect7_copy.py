...
@sio.on('vehicle-speed')
def on_vehicle_speed(data):
    global current_vehicle_speed
    current_vehicle_speed = data.get('speed', 0)
    print(f"🚗 Current Vehicle Speed: {current_vehicle_speed} km/h")
...
def on_manual_speed_alert(data):
    ...
    try:
        ...
        current_vehicle_speed = int(speed_val)
        print(f"📡 Manual Speed Updated: {current_vehicle_speed} km/h")
    except (ValueError, TypeError):
        print(f"⚠️ Invalid speed received: {data.get('speed')}")
    ...
    if chosen:
        ...
        # Regular alert logic for other signs
            ...
            if "speed limit" in chosen:
                speed_limit = get_speed_from_label(chosen)
                if speed_limit:
                    if current_vehicle_speed > speed_limit:
                        print(f"⚠️ SPEEDING: Vehicle Is at {current_vehicle_speed} km/h, Limit is {speed_limit} km/h")
                        speak_async(f"Attention: Your speed is {current_vehicle_speed} kilometers per hour. The limit is {speed_limit}. Please lower your speed.")
                    else:
                        print(f"✅ Safe: Speed {current_vehicle_speed} km/h, Limit {speed_limit} km/h")
            else:
